...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=None, use_checkpoint=False, small_kernel_merged=False,out_indices=[0,1,2,3],use_sync_bn=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024],out_indices=out_indices,use_sync_bn=use_sync_bn,
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,in_channels=2,
                    small_kernel_merged=small_kernel_merged)

def create_RepLKNet31L(drop_path_rate=0.3, num_classes=None, use_checkpoint=False, small_kernel_merged=False,out_indices=[0,1,2,3],use_sync_bn=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[192,384,768,1536],out_indices=out_indices,use_sync_bn=use_sync_bn,
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,in_channels=2,
                    small_kernel_merged=small_kernel_merged)

def create_RepLKNetXL(drop_path_rate=0.3, num_classes=None, use_checkpoint=False, small_kernel_merged=False,out_indices=[0,1,2,3],use_sync_bn=False):
    return RepLKNet(large_kernel_sizes=[27,27,27,13], layers=[2,2,18,2], channels=[256,512,1024,2048],out_indices=out_indices,use_sync_bn=use_sync_bn,
                    drop_path_rate=drop_path_rate, small_kernel=None, dw_ratio=1.5,
                    num_classes=num_classes, use_checkpoint=use_checkpoint,in_channels=2,
                    small_kernel_merged=small_kernel_merged)

if __name__ == '__main__':
    model = create_RepLKNet31B(use_sync_bn=False)
    pretrained = torch.load('/home/wangjingqi/input/ck/g2net/pretrained/RepLKNet-31B_ImageNet-1K_224.pth', map_location='cpu')
    model_param = model.state_dict()
    pretrained_param = {}
    for k,v in pretrained.items():
        if k in model_param:
            if v.shape != model_param[k].shape:
                print('pop', k, v.shape, model_param[k].shape)
            else:
                pretrained_param[k] = v
        else:
            print('pop', k, v.shape)
    model.load_state_dict(pretrained_param, strict=False)
    model.eval()
    print('------------------- training-time model -------------')
    print(model)
    x = torch.randn(2, 2, 300, 224)
    origin_y = model(x)
    for o in origin_y:
        print(o.shape)
