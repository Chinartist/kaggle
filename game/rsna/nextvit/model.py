...
class NextViT(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x, 1)
        return x
def nextvit_base(pretrained=False, pretrained_path=None, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3],**kwargs)
    if pretrained:
        print("load pretrained model from {}".format(pretrained_path))
        model.load_state_dict(torch.load(pretrained_path,map_location="cpu")["model"], strict=False)
    return model
...
class NextVitBNet(nn.Module):
    def __init__(self, **model_kwargs):
        ...
        self.encoder = nextvit_base(pretrained=True,pretrained_path="/home/wangjingqi/input/ck/rsna/nextvit_base_in1k6m_384.pth",**model_kwargs)
...
class seresnext(nn.Module):
    ...
    def __init__(self, **model_kwargs):
        super().__init__()
        self.encoder = timm.create_model("seresnext50_32x4d", 
                                          pretrained=True,)
        self.encoder._modules['fc'] = torch.nn.Identity()
        self.register_buffer('mean', torch.FloatTensor([0.5, 0.5, 0.5]).reshape(1, 3, 1, 1))
        self.register_buffer('std', torch.FloatTensor([0.5, 0.5, 0.5]).reshape(1, 3, 1, 1))
