...
import math
import os  # TODO(chore): change all os calls to pathlib
...
mit = {0: mit_b0, 2: mit_b2,3: mit_b3 ,4:mit_b4,5: mit_b5}

from mutisampledropout import MultiSampleClassifier

class segformer(nn.Module):
    def __init__(self,config = None,pretrain=True ) -> None:
        super().__init__()
        self.encoder = mit[config.mit]()
        if pretrain:
            checkpoint_model = torch.load(os.path.join(config.pretrain, f"mit_b{config.mit}.pth"),map_location="cpu")
    ...
    def forward(self, x: Tensor,ogs) -> Tensor:
        # stem and backbone
...
# model=segformer(config,pretrain=False)
